export const meta = {
  ...
  phases: [
    ...
    { title: 'Plan.enrich' },
    { title: 'Plan.critic' },
    { title: 'Plan.verify.correctness' },
    { title: 'Plan.verify.synthesis' },
    { title: 'Plan.correct.revise' },
...
const r_investigate_survey = await agent(
  `<instructions>
...
`,
  { label: 'investigate.survey', phase: 'Investigate.survey', model: mInvestigateSurvey },
)
if (!r_investigate_survey) throw new Error('investigate.survey agent failed')

phase('Investigate.findings')
log('Running investigate.findings steps…')

const r_investigate_findings = await agent(
  `<instructions>
...
`,
  { label: 'investigate.findings', phase: 'Investigate.findings', model: mInvestigateFindings },
)

phase('Plan.intake')
log('Running plan.intake steps…')

const r_plan_intake = await agent(
...
Codebase survey: ${r_investigate_findings}
`,
  { label: 'plan.intake', phase: 'Plan.intake', model: mPlanIntake },
)

phase('Plan.enrich')
log('Running plan.enrich steps…')

const r_plan_enrich = await agent(
...
${r_plan_intake}
`,
  { label: 'plan.enrich', phase: 'Plan.enrich', model: mPlanEnrich },
)

phase('Plan.critic')
log('Running plan.critic steps…')

const [plan_criticRes, plan_securityRes] = await parallel([
  () =>
    agent(
...
${r_plan_enrich}
`,
      { label: 'plan.critic', phase: 'Plan.critic', model: mPlanCritic, schema: WEAK_SCHEMA },
    ),
  () =>
    agent(
...
${r_plan_enrich}
`,
      { label: 'plan.security', phase: 'Plan.critic', model: mPlanSecurity, schema: SEC_SCHEMA },
    ),
])
...
const _assembleInput = {
  'investigate.survey': r_investigate_survey,
  'investigate.findings': r_investigate_findings,
  'plan.intake': r_plan_intake,
  'plan.enrich': r_plan_enrich,
  'plan.critic': plan_criticRes,
  'plan.security': plan_securityRes,
  'plan.verify.correctness': r_plan_verify_correctness,
  'plan.verify.security': r_plan_verify_security,
  'plan.verify.synthesis': r_plan_verify_synthesis,
  'plan.correct.revise': r_plan_correct_revise,
  'build.develop.implement': r_build_develop_implement,
  'build.review.correctness': r_build_review_correctness,
  'build.review.security': r_build_review_security,
  'build.review.synthesis': r_build_review_synthesis,
}
...
phase('Plan.verify.correctness')
log('Running plan.verify.correctness steps…')

const [plan_verify_correctnessRes, plan_verify_securityRes] = await parallel([
  () =>
    agent(
      `<instructions>
...
`,
      { label: 'plan.verify.correctness', phase: 'Plan.verify.correctness', model: mPlanVerifyCorrectness },
    ),
  () =>
    agent(
      `<instructions>
...
`,
      { label: 'plan.verify.security', phase: 'Plan.verify.correctness', model: mPlanVerifySecurity },
    ),
])


phase('Plan.verify.synthesis')
log('Running plan.verify.synthesis steps…')

const r_plan_verify_synthesis = await agent(
  `<instructions>
...
${r_plan_verify_correctness}

Security audit:

${r_plan_verify_security}
</context>
...
`,
  { label: 'plan.verify.synthesis', phase: 'Plan.verify.synthesis', model: mPlanVerifySynthesis },
)

phase('Plan.correct.revise')
log('Running plan.correct.revise steps…')

const r_plan_correct_revise = await agent(
  `<instructions>
...
`,
  { label: 'plan.correct.revise', phase: 'Plan.correct.revise', model: mPlanCorrectRevise },
)
...
const r_build_develop_implement = await agent(
  `<instructions>
...
`,
  { label: 'build.develop.implement', phase: 'Build.develop.implement', model: mBuildDevelopImplement },
)
...
const [build_review_correctnessRes, build_review_securityRes] = await parallel([
  () =>
    agent(
      `<instructions>
You are an adversarial correctness reviewer. You are given the change under review — a plan, a diff, or source code — and read access to the repository it applies to. Your job is to find every place where it is wrong, incomplete, or inconsistent with the codebase.
...
`,
      { label: 'build.review.correctness', phase: 'Build.review.correctness', model: mBuildReviewCorrectness },
    ),
  () =>
    agent(
      `<instructions>
...
`,
      { label: 'build.review.security', phase: 'Build.review.correctness', model: mBuildReviewSecurity },
    ),
])
...
const r_build_review_synthesis = await agent(
  `<instructions>
You are a senior reviewer synthesising two independent audits of the same plan. Your job is to merge the findings into a single, prioritised, deduplicated list that a developer can act on.
</instructions>
...
${r_build_review_correctness}

Security audit:

${r_build_review_security}
</context>
