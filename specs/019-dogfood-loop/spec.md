...
> **Amendment (2026-09-21):** By owner directive no workflow may push or open a pull request; `ship`'s `pr` step was removed from `pipelines/ship.yaml` (ADR-0019 amendment). FR-002's second and third sentences described that step as a live safety property — they are amended below. The sandbox's missing `origin` remote (FR-002's first sentence) still stands as the outer guard.
...
| FR-002 | The sandbox has no `origin` remote, so nothing in it can reach GitHub; its single remote `base` points at the local base checkout and exists only so the sandbox can be re-synced. The `ship` stage's `approve` gate is `manualOnly: true`, so a commit never happens without the owner answering. Amended 2026-09-21: `ship` no longer has a `pr` step at all, so no stage can open a pull request from the sandbox or anywhere else. |
...
3. **Create Run** — POST to `/api/runs` with the spec file path (e.g., `specs/016-provider-profiles/spec.md`) as the input request. This seeds the `cycle-dev` pipeline, which runs the plan→develop→build→review chain described in the overview; the full `cycle` pipeline adds a `ship` stage that commits locally (amended 2026-09-21: it no longer opens a pull request), which is outside the dogfood loop.
