...
/**
 * Everything `install`, `uninstall` and `doctor`'s reach report depend on.
 *
 ...
 * that OpenCode also reads, so `install` refuses instead (D10). Neither can be
...
/**
 * One harness, as `install`, `uninstall` and the reach report all see it.
 *
 * `addArgs`/`removeArgs` are absent for a harness whose configuration `install`
...
function setupOpencode(env: SetupEnv, harness: Harness): HarnessResult {
  ...
  // once: a second install must not overwrite the pre-agent-flows copy with a
...
/** The copy `install` takes of an OpenCode config before its first edit. */
 ...
 * that `install` itself created — the case with no `.bak` beside it — is deleted
 * outright, and the backup it took goes once the file it restores matches it
 * again. All three exist so that install-then-uninstall leaves the directory exactly
...
function removeOpencode(env: SetupEnv, harness: Harness): HarnessResult {
  ...
  // already had stays, even when it is now empty; one install introduced goes.
  const backup = backupPath(configPath);
  ...
  if (!hadBackup && Object.keys(parseJsonText(next) ?? {}).length === 0) {
    rmSync(configPath);
    return result("removed", `removed ${configPath}, which install had created`, configPath);
  }
...
/**
 * Drop the backup `install` took, now that the entry it was taken for is gone.
 *
 * Only when the restored file matches it byte for byte: a file edited between
 * install and uninstall differs, and then the backup is the only copy of what was
