// Spec 038 Ship 4: `agent-flows install`, `agent-flows uninstall` and doctor's reach
...
function makeHome(): string {
  const home = mkdtempSync(join(tmpdir(), "agent-flows-harness-"));
  homes.push(home);
  return home;
}
 ...
 * that changed during install is one install itself wrote.
 */
function fakeEnv(
  home: string,
...
// ── FR-030: install registers each installed harness ─────────────────────────

describe("FR-030: install registers the MCP server", () => {
  ...
  it("copies the OpenCode config to .bak once, before the first edit", () => {
    ...
    // A second install must not replace the pre-agent-flows copy.
...
describe("FR-030: install is idempotent", () => {
  it("a second install changes nothing and reports every harness already registered", () => {
    const home = makeHome();
    ...
    assert.deepEqual(
      ...
      [],
      "a second install must write nothing"
    );
...
// ── FR-030 / D17: install writes to no other path ────────────────────────────

describe("D17: install touches nothing that belongs to agent-notes", () => {
  ...
  it("changes only the OpenCode config it edits, and no harness dot-directory file", () => {
    ...
    // The fakes only record: every file that changed was written by install itself.
    runSetup(fakeEnv(home).env);
    ...
    assert.deepEqual(
      ...
      "install may write its own MCP entry and nothing else"
    );
...
describe("FR-031: uninstall reverses exactly what install wrote", () => {
  ...
  it("leaves the whole OpenCode config directory as it found it, formatting included", () => {
    ...
    assert.notEqual(
      ...
      original,
      "install must actually have edited the file, or this test proves nothing"
    );
  ...
  it("keeps the backup when the config changed after install, and says where it is", () => {
    const home = makeHome();
    const original = JSON.stringify({ theme: "tokyonight" }, null, 2) + "\n";
    writeFile(opencodeConfigPath(home), original);
    runSetup(fakeEnv(home, { emulate: true }).env);
  ...
  it("deletes an OpenCode config that install itself created", () => {
    const home = makeHome();
    runSetup(fakeEnv(home, { emulate: true }).env);
    assert.ok(readOpencodeRegistration(home), "install should have created the config");
...
describe("FR-032: harnessReach", () => {
  ...
  it("flags a registration whose command no longer exists as stale", () => {
    ...
    // The package is removed without `agent-flows uninstall` first: every harness
