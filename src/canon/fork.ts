...
//
// A fork copies exactly one workflow's YAML and its own prompt files, never the
// transitive closure of its nested pipelines: the merged view (D13) resolves a
// mount across layers, so a forked parent keeps mounting the children it already
// mounts, wherever they live.
//
...
import { layerFrom, mergedCatalog, repoCanonRoot, userLibraryRoot } from "./layers.js";
import { readRawPipeline } from "./rawPipeline.js";
import type { CanonLayer, CatalogEntry, MergedCatalog } from "./layers.js";
...
/**
 * The layer `agent-flows fork <id>` writes into.
 *
 * An explicit `--to` wins. Otherwise the repository canon when the project
 * already has one — which is exactly when `layers` carries a repo layer — else
 * the user library: a project that has chosen to keep workflows in the tree
 * keeps them there, and one that has not does not acquire a committed directory
 * as a side effect of editing.
 *
 * A layer the project does not have yet is absent from `layers`; the fork
 * creates its directories, so it is constructed here instead.
 */
export function resolveForkTarget(
  ...
  const present = (source: ForkTarget): CanonLayer | undefined =>
    layers.find((l) => l.source === source);
  if (target === undefined) return present("repo") ?? layerFrom("user", userLibraryRoot(env));
  const root = target === "repo" ? repoCanonRoot(projectDir) : userLibraryRoot(env);
  return present(target) ?? layerFrom(target, root);
}
...
function ownPromptPaths(yamlPath: string): string[] {
  ...
  for (const step of readRawPipeline(yamlPath).steps ?? []) {
...
export function forkPipeline({ id, catalog, target, overwrite }: ForkOptions): ForkReport {
  ...
  const destYaml = join(target.pipelinesDir, `${id}.yaml`);

  if (existsSync(destYaml) && !overwrite) {
    throw new Error(
      `agent-flows: "${id}" already exists in the ${target.source} layer at ${destYaml} — ` +
  ...
  mkdirSync(target.pipelinesDir, { recursive: true });
  mkdirSync(target.promptsDir, { recursive: true });
  ...
  return { id, from: entry.layer.source, target: target.source, root: target.root, written };
}
