...
Then a section headed exactly `Constraints relied on`. It is required and it is never empty: every trace you ran ended somewhere, and this is where you say what held the value on the way. One entry per value the change handles that a user, a caller, a configuration file or a stored record can influence:

- **Value** — the value and the operation it reaches, stated so the claim could be false.
- **Constrained at** — the layer that bounds it and the line you read there: a validation, a type, a schema, an allow-list, an escape or a check at the boundary. Cite the file and quote it.
- **Or `constrained nowhere`** — when you traced every layer you can reach and no line bounds it. Name the searches that came back empty.

`constrained nowhere` is a finding, not an omission: raise it in the list above too, with its location and severity. A constraint applied only inside the operation that consumes the value does not bound the value arriving from anywhere else.

Then a line naming the framing you were given and the one value outside it you traced.

The review is done when every part of the change that touches authentication, input handling, data storage or external interfaces has been assessed, every blast-radius entry that carries a value has been traced, and every value in `Constraints relied on` carries either a quoted constraining line or `constrained nowhere`.
</output_format>
